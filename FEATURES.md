...
| Editor (screen 06) | ✅ | `/projects/[id]/edit`: reorder clips (up/down), remove, **music picker** (from catalog), **length** (15/30/60), **draft preview player**, **Render HD** + live status + **Download** (proxied). |
...
| Music catalog | 🚧 | 3 **placeholder** instrumental beds seeded (BPM/mood, in MinIO `music/`). ⚠️ `license_ref=PLACEHOLDER-DO-NOT-SHIP` — replace with real licensed royalty-free tracks before launch. Seeder is now **manifest-driven** (`scripts/seed-music.mjs` + `music-manifest.example.json`) and **refuses placeholder licence refs**; sourcing + licensing decision + go-live checklist in `MUSIC_CATALOG.md`. Playback via proxied `GET /api/music/[trackId]`. |
