...
| 360 reframe modes (AutoReframe) | ✅ | Per-clip **360 view** in the timeline clip editor (restored 2026-09-25): **Follow action** (default for new uploads — motion-tracked yaw path, circular smoothing, rate-limited pan), **Front** (steady lens-axis/levelled view) and **Tiny planet** (2-lens files). Changing it re-converts the source (`setClipReframe`, renders wait for `ready`). **Single-lens files** (Insta360 split recordings — one lens per `_00_`/`_10_` file) are projected along the lens axis without the full-sphere auto-level (which mis-aimed them: "tilt 96°", black edge + upside-down deck); follow stays within ±45° so the view never leaves the lens. Flat output uses an aspect-correct vertical FOV (was squashed ~1.3×). Worker: `reframeGraph`; diagnostic `--followtest <file> [flat|follow|tiny] [secs]`. |
...
| Export / share (watermark) | ✅ | Download (direct, signed). **Watermark on every video** (renders, AI generations, enhancements, storyboards, exports) = the ClipWaltz logo, bottom-left; Free always, paid plans per the `/admin` switch (default on). AI outputs keep a clean master (`clean_key`) so Enhance/Assemble/Export never stack the logo. **Share links:** Public/Unlisted renders get `/w/<id>` (Copy link in the render panel); private ones 404. |
