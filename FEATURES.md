...
| Projects board (organise) | ✅ | `/projects` is a **card board** (`projects-board.tsx`): **uniform, wider cards** (`auto-fill minmax(210px)`) with a fixed 16:9 thumbnail and an **orientation icon** (portrait/landscape) so the grid stays even and names are readable. **Search** box (name + tags), a **tag filter** row (`projects.tags`, AND-filter), and **user-created categories** (`projects.category`) rendered as sections. **Click-hold drag** a card between category sections to move it (HTML5 DnD → `setProjectCategory`, optimistic); also a card-menu **Move to** submenu + **Edit tags…**. Empty user categories persist per-browser (localStorage) as drop targets until filled. Migration 0022. |
