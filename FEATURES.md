...
| Cloud auto-assemble | ✅ | Normalize to 9:16/16:9 + concat/crossfade + Ken Burns + filters + title + fades + music + length cap + watermark. **Beat-synced cuts & smart active-moment/face-scene selection = later phase.** |
...
| Music catalog | ✅ (dev) | **6 real licensed Pixabay beds** seeded in `clipwaltz_dev` (2 each upbeat/chill/cinematic, Pixabay Content License); placeholders retired. Seeder is **manifest-driven** (`scripts/seed-music.mjs` + `music-manifest.example.json`) and **refuses placeholder licence refs**; sourcing + go-live checklist in `MUSIC_CATALOG.md`. Playback via proxied `GET /api/music/[trackId]`. **Prod DB still needs the same seed run before launch.** |
