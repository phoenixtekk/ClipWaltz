...
| Per-clip screen time | ✅ | Set **how long each image/video shows** (`assets.durationOverride`, 0024): open a clip from the Timeline strip → a modal **plays that specific clip** (scrub/preview) with a **manual time slider + number** (0.4–60s; videos clamped to source length) or **Auto**. The timeline block widths + labels reflect the manual time (violet badge). Worker: overridden slots use the exact time and are pinned (excluded from stretch-to-fill). `setAssetDuration`. |
