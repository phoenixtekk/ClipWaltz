...
| Insta360 / 360 import | ✅ | Accepts **`.insv`/`.lrv`/`.insp`** uploads; the worker **reprojects 360 footage to a flat, shareable, auto-levelled clip** with ffmpeg `v360` (dual-fisheye→`dfisheye` with `roll=90` leveling, single→`fisheye`; keeps audio) — no Insta360 Studio. Reprojection + leveling visually verified on a real bright dual-fisheye clip. Async conversion queue on the assets (`sourceFormat`/`conversionState`/`convertedKey`, 0011); editor shows a "converting" state and auto-refreshes; render/preview use the converted clip. Verified E2E on a real `.insv` (`--convtest`). Advertised on the home page. |
...
| Timeline (view + insert/reorder) | ✅ | Full-video **timeline** (`project-timeline.tsx`): clips laid left→right, widths scaled by draft duration; **drag a clip to reorder**, **+ between clips to insert/upload** an image or video at that exact spot (with progress), remove per clip. `reorderAssets` server action. |
